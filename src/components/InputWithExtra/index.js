...
export default class InputWithExtra extends Component {
	...
	render() {
		const {firstName, firstTitle, secondName, secondTitle} = this.props;
		...
		return (
			<div className={IWEStyle["whole-shit"]}>
				<div className={IWEStyle["first-part"]}>
					...
					<input name={firstName || ""} className={IWEStyle.input} type="text" value={inputVal} onChange={(event) => this.setState({"inputVal": event.target.value})}/>
				</div>
				{
					(inputVal.length)?
